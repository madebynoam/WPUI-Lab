...
## Task 3: Create Viewport Controls Component

**Requirements**: 1.1, 1.2, 1.3, 1.4, 2.1, 2.2, 2.3, 2.4, 7.1, 7.2, 7.3, 7.4, 7.5

**Description**: Create toolbar component with viewport preset buttons and zoom controls.
...
- Create functional component `ViewportControls`
- Import context: `useComponentTree`
- Get state: `viewportPreset`, `zoomLevel`, `isPlayMode`
- Get actions: `setViewportPreset`, `setZoomLevel`
- Return null if `isPlayMode` is true

### 3.2: Add Viewport Preset Buttons
- Create button for Mobile (375px) with emoji 📱
- Create button for Tablet (768px) with emoji 📱
- Create button for Desktop (1440px) with emoji 🖥️
- Create button for Full Width with emoji ↔️
- Highlight active preset with blue background (#3858e9)
- Add tooltips with keyboard shortcuts (Cmd+1/2/3/0)

### 3.3: Add Zoom Controls
- Define `ZOOM_LEVELS` constant: `[0.5, 0.75, 1.0, 1.5, 2.0]`
- Create zoom out button (−) with handler
- Create zoom percentage display button (shows current zoom, resets on click)
- Create zoom in button (+) with handler
- Disable zoom out at minimum (50%)
- Disable zoom in at maximum (200%)
- Add tooltips with keyboard shortcuts (Cmd+Plus/Minus/Shift+0)

### 3.4: Style Toolbar
- Use flexbox layout with gap
- Add white background with bottom border
- Add padding: 8px 12px
- Separate viewport and zoom controls with border
- Use consistent button styling (border, border-radius, padding)

---

## Task 4: Create Viewport Frame Component
...
### 4.3: Add Constrained Viewport Mode
...
## Task 5: Integrate Viewport Controls into Canvas

**Requirements**: 7.1, 7.6

**Description**: Add ViewportControls toolbar and ViewportFrame wrapper to Canvas component.
...
- Import `ViewportControls` component
- Import `ViewportFrame` component

### 5.2: Add Viewport Controls Toolbar
- Add `<ViewportControls />` at top of canvas layout
- Position above canvas content area
- Ensure it's outside ViewportFrame (not zoomed)

### 5.3: Wrap Canvas Content with ViewportFrame
- Wrap existing canvas content div with `<ViewportFrame>`
- Move all canvas rendering logic inside ViewportFrame
- Ensure breadcrumb and viewport indicator stay outside frame

### 5.4: Update Layout Structure
- Maintain flex column layout
- Ensure ViewportControls doesn't scroll with canvas
- Ensure ViewportFrame contains scrollable canvas content
...
## Task 7: Add Coordinate Scaling for Interactions
...
### 7.1: Create Coordinate Scaling Helper
...
### 7.2: Update Mouse Event Handlers
...
## Task 8: Update Viewport Indicator
...
### 8.1: Import Viewport State
- Import `viewportPreset` from ComponentTreeContext
- Import `useResponsiveViewport` hook

### 8.2: Update Breakpoint Display
...
### 8.3: Add Viewport Width Display
...
## Task 9: Handle Play Mode Behavior
...
### 9.1: Hide Controls in Play Mode
- Check `isPlayMode` in ViewportControls
- Return null if play mode is active
- Ensure toolbar doesn't render in play mode

### 9.2: Force Full Width in Play Mode
...
**Files to Create**:
- `src/components/ViewportControls.test.tsx`
...
### 10.1: Test Viewport Controls
- Test viewport preset button clicks
- Test zoom in/out button clicks
- Test zoom reset button click
- Test button disabled states at boundaries
- Test active preset highlighting
- Test play mode hiding

### 10.2: Test Viewport Frame
...
### 10.3: Test Responsive Viewport Hook
...
## Task 11*: Add Viewport Rotation (Optional)
...
## Task 12*: Add Custom Viewport Width Input (Optional)
...
3. **Task 3**: Create viewport controls component (UI)
4. **Task 4**: Create viewport frame component (UI)
5. **Task 5**: Integrate into Canvas (integration)
6. **Task 8**: Update viewport indicator (visual feedback)
7. **Task 6**: Add keyboard shortcuts (UX enhancement)
8. **Task 7**: Add coordinate scaling (interaction fix)
9. **Task 9**: Handle play mode behavior (edge case)
10. **Task 10**: Add tests (validation)
11. **Task 11***: Add viewport rotation (optional)
12. **Task 12***: Add custom viewport width (optional)
